...
| `syntect` (+ bundled Sublime syntaxes) | Tristan Hume & contributors | MIT | <https://github.com/trishume/syntect> | The broad "regex tier" of the hybrid highlighter (PRD §5.7): drives dozens of languages from compact `.sublime-syntax` data via the pure-Rust `regex-fancy` backend. Its bundled default `.sublime-syntax` definitions carry their own upstream licenses, surfaced by syntect's own acknowledgements. Integrated M1. |
